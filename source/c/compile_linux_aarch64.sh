...
echo "JHDF5 building..."
gcc -shared -O3 -fPIC -Wl,--exclude-libs,ALL jni/*.c -Ihdf5-${VERSION}-aarch64/include -I$JAVA_HOME/include -I$JAVA_HOME/include/linux hdf5-${VERSION}-aarch64/lib/libhdf5.so -o libjhdf5.so -lz &> jhdf5_build.log

if [ -f "hdf5-${VERSION}-aarch64/lib/libhdf5.so" ]; then
  mkdir -p ../../../libs/native/hdf5/aarch64-Linux
  cp -pf "hdf5-${VERSION}-aarch64/lib/libhdf5.so" ../../../libs/native/hdf5/aarch64-Linux/
